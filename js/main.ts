document.addEventListener('DOMContentLoaded', function () {
    ...
    cy.on('taphold', function (e: Event) {
        let ele = e.target;
        if (ele.selected()) {
            ele.unselect()
        } else {
...
function delElements(cy: any) {
    let eles = cy.$(":selected").remove()
    animate(cy)
    return eles
}

function printPNG(cy: any) {
